...
export function ManageShipment() {
    ...
    const warehouseForm = useForm<ConfirmFormData>({
    ...
    const qualityForm = useForm<ConfirmFormData>({
        resolver: zodResolver(confirmSchema),
        defaultValues: {
            shipmentCode: "",
        },
    ...
    const onWarehouseConfirm = async (data: SimpleActionFormData) => {
        try {
            console.log(`Warehouse confirming shipment: ${data.shipmentCode}`);
            warehouseConfirm(data.shipmentCode);
            toast.success(`Warehouse confirming shipment ${data.shipmentCode}...`);
        } catch (error) {
            console.error("Error warehouse confirming:", error);
            toast.error(`Error occurred while warehouse confirming: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    };

    const onQualityApprove = async (data: SimpleActionFormData) => {
        try {
            console.log(`Quality approving shipment: ${data.shipmentCode}`);
            qualityApprove(data.shipmentCode);
            toast.success(`Quality approving shipment ${data.shipmentCode}...`);
        } catch (error) {
            console.error("Error quality approving:", error);
            toast.error(`Error occurred while quality approving: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    };
    ...
    useEffect(() => {
        if (warehouseError) {
            toast.error("Warehouse confirm error: " + warehouseError.message);
        }
    }, [warehouseError]);

    useEffect(() => {
        if (qualityError) {
            toast.error("Quality approve error: " + qualityError.message);
        }
    }, [qualityError]);
                    ...
                    <Tabs defaultValue="update" className="w-full">
                        ...
                        <TabsContent value="warehouse" className="space-y-4">
                            <Card>
                                ...
                                <CardContent>
                                    <Form {...warehouseForm}>
                                        <form onSubmit={warehouseForm.handleSubmit(onWarehouseConfirm)} className="space-y-4">
                                            <FormField
                                            ...
                                            <Button
                                                type="submit"
                                                className="w-full"
                                                disabled={isWarehouseConfirming || isConfirmingWarehouse}
                                            >
                                                {isWarehouseConfirming || isConfirmingWarehouse ? (
                                                    <>
                                                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        ...
                        <TabsContent value="quality" className="space-y-4">
                            <Card>
                                ...
                                <CardContent>
                                    <Form {...qualityForm}>
                                        <form onSubmit={qualityForm.handleSubmit(onQualityApprove)} className="space-y-4">
                                            <FormField
                                            ...
                                            <Button
                                                type="submit"
                                                className="w-full"
                                                disabled={isQualityApproving || isConfirmingQuality}
                                            >
                                                {isQualityApproving || isConfirmingQuality ? (
                                                    <>
                                                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                                        Quality Approving...
                                                    </>
